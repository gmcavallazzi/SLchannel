...
Usage: python scripts/model_floor_1d.py   (CPU, seconds)
Output: figures_fix/model_floor_1d.png
"""
...
def main():
    dt = CFL_FRAC * H / C0
    u0 = np.real(np.fft.irfft(
        np.where(np.arange(len(k)) <= K_FORCE,
    ...
    print(f"{'dt/tau':>7} {'ab2':>10} {'none':>10} {'pc':>10} {'bdf2':>10}   (tail energy / forced-band energy)")
    ...
    fig.suptitle('1-D model: equilibrium spectrum vs the noise-free remap '
                 f'(CFL={CFL_FRAC}, $\\zeta_{{rms}}/c_0$={ZETA_REL})', fontsize=11)
    fig.tight_layout()
    os.makedirs('figures_fix', exist_ok=True)
    fig.savefig('figures_fix/model_floor_1d.png', dpi=150)
    print("saved figures_fix/model_floor_1d.png")

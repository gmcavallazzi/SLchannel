...
Usage: python scripts/refinalize_stats.py <config.yaml> <results_folder>
"""
...
def main():
    ...
    z_f, z_c, dz_f, dz_c = generate_grid(
        f['gamma'], g['nz'], d['Lz'],
        stretching_type=d.get('stretching_type', 'symmetric'))
    ts = TurbulenceStats(
        g['nx'], g['ny'], g['nz'], d['Lx'], d['Ly'], d['Lz'],
        z_c, z_f, dz_c, dz_f, d['Lx'] / g['nx'], d['Ly'] / g['ny'],
        1.0 / f['Re'], f['Re_tau'],
        z_plus_target=s.get('z_plus_target', 15.0), device='cpu',
        top_wall_bc_type=cfg.get('boundary_conditions', {})
                            .get('top_wall', {}).get('type', 'dirichlet'))

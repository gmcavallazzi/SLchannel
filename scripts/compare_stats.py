...
Usage:
    python scripts/compare_stats.py run1.npz [run2.npz ...] \
...
def main():
    ...
    for path, lab in zip(args.files, labels):
        ...
        ax_vw.plot(zp, np.sqrt(np.maximum(s['vv_mean'][half], 0)) / u_tau, '-',
                   label=f"{lab} v'")
        ax_vw.plot(zp, np.sqrt(np.maximum(s['ww_mean'][half], 0)) / u_tau, '--',
                   label=f"{lab} w'")
    ...
    ax_u.semilogx(zp_ref[zp_ref > 25], np.log(zp_ref[zp_ref > 25]) / 0.41 + 5.2,
                  'k:', lw=1, label='log law')

    ax_u.set_xlabel(r'$z^+$'); ax_u.set_ylabel(r'$U^+$')
    ax_uu.set_xlabel(r'$z^+$'); ax_uu.set_ylabel(r"$u'_{rms}/u_\tau$")
    ax_vw.set_xlabel(r'$z^+$'); ax_vw.set_ylabel(r"$v'_{rms}, w'_{rms}/u_\tau$")
    ax_uw.set_xlabel(r'$z^+$'); ax_uw.set_ylabel(r"$-\overline{u'w'}/u_\tau^2$")

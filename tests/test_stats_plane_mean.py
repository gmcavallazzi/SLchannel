...
def run():
    ...
    z_f, z_c, dz_f, dz_c = generate_grid(1.5, nz, Lz, device='cpu',
                                         stretching_type='symmetric')
    ts = TurbulenceStats(nx, ny, nz, Lx, Ly, Lz, z_c, z_f, dz_c, dz_f,
                         Lx / nx, Ly / ny, 1e-3, 180.0,
    ...
    ok &= report("<u'w'> unaffected", abs(float(stats['uw_mean'][nz // 2])) < 1e-14,
                 f"uw={float(stats['uw_mean'][nz // 2]):.2e}")
    ok &= report("w'w' zero for zero w", abs(float(stats['ww_mean'][nz // 2])) < 1e-14,

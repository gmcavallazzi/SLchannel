...
def run():
    ...
    fill_mid(adv, z_f, z_c,
             lambda X, Y, Z: U0 + 0 * X, lambda X, Y, Z: V0 + 0 * X,
             lambda X, Y, Z: W0 + 0 * X)
    dt = torch.tensor(0.05)
    for comp in 'uvw':
        xa, ya, za, _ = adv.arrival[comp]
        xd, yd, zd = adv.departure_coords(comp, dt)
        err = max((xd - (xa - dt * U0)).abs().max().item(),
                  (yd - (ya - dt * V0)).abs().max().item(),
                  (zd - (za - dt * W0)).abs().max().item())
        ok &= report(f"uniform translation, comp={comp}", err < 1e-13, f"err={err:.2e}")

    # ---- linear shear u = S*z: exact (velocity constant on trajectory) --
    S = 0.8
    fill_mid(adv, z_f, z_c,
             lambda X, Y, Z: S * Z, lambda X, Y, Z: 0 * X, lambda X, Y, Z: 0 * X)
    xa, ya, za, _ = adv.arrival['u']
    xd, yd, zd = adv.departure_coords('u', dt)
    err = max((xd - (xa - dt * S * za)).abs().max().item(),
              (yd - ya).abs().max().item(), (zd - za).abs().max().item())
    ok &= report("linear shear", err < 1e-13, f"err={err:.2e}")
    ...
    ok &= report("rotation local error O(dt^3)", r1 > 6.0 and r2 > 6.0,

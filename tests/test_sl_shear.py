...
def shear_error(nx, order):
    ...
    ustar, vstar, wstar = adv.advect(
        fields["u"], fields["v"], fields["w"], mid_u, mid_v, mid_w, torch.tensor(DT)
    )
...
def test_sl_shear(check):
    e_cubic = shear_error(64, 4)
    check("shear, tricubic nx=64", e_cubic < 1e-5, f"err={e_cubic:.3e}")

    e1, e2 = shear_error(32, 4), shear_error(64, 4)
    check(
        "shear convergence O(h^4)",
        e1 / e2 > 10.0,
    ...
    check(
        "shear, triquintic beats tricubic",
        e_quintic < e_cubic / 10,

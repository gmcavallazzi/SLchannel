...
Run:  SLCHANNEL_COMPILE=1 CC=gcc PYTORCH_JIT=0 python benchmarks/bench_interp.py [nx ny nz]
...
live in benchmarks/bench_step.py (2026-07-04: Eulerian IMEX step ~1.29 s with
...
def main():
    ...
    print(
        f"Grid: {NX} x {NY} x {NZ} ({NX * NY * NZ / 1e6:.1f} M pts/component); "
        f"performance layers: {env.summary()}\n"
    )
    ...
    cases = [
        ("triton", 4, "fp32_accum64", "1"),
        ("triton", 6, "fp32_accum64", "1"),
        ("inductor", 4, "fp32_accum64", "0"),
        ("inductor", 4, "fp64", "0"),
    ]
    for tier, order, interp_dtype, triton_flag in cases:
        # env flags are cached at import; re-read after mutating the
        # environment or the toggle is a no-op.
        ...
        if tier == "triton" and adv._triton is None:
            ...
            continue
        t = timeit(lambda a=adv: a.advect(u, v, w, u, v, w, dt_t), device)

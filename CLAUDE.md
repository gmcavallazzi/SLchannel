...
- `sl.traj_extrapolation` (v1/v2 only): `'ab2'` (default) | `'none'` (diagnostic, O(dt) trajectories) | `'pc'` — predictor-corrector mid-velocity; the predictor **must be projected** before averaging (one extra Poisson/step; unprojected u* has O(dt) divergence → compressive trajectories → u_τ blow-up, fixed 2026-08-03; self-convergence ratio 3.78).
- `sl.time_scheme: "bdf2"` — Boukir et al. (1997) BDF2 characteristics (`step_sl_bdf2`): ONE characteristic over [t^{n−1},t^{n+1}] traced with frozen U* = 2Vⁿ−Vⁿ⁻¹, two independent feet at depths dt and 2dt (never continue the far foot from the near foot — order drops to 1, paper Remark 4i), update (3u^{n+1}−4ū+ū̄)/2dt with θ=1 z-solve and projection at dt_eff = 2dt/3. `bdf2_pressure: noninc|inc` (inc = O(dt²), ratio 4.00; noninc robust, ratio 2.0), `bdf2_xy_rhs: extrap|lagged`. Constant dt assumed — BDF1 re-bootstrap on first step/restart/dt change; **pin dt** (`dt_update_interval: 0`) in bdf2 configs. Motivation: BDF2 damps high-frequency content that v2's trajectory-CN remaps neutrally — candidate cure for the M3 spectral floor (validation sweep 2026-08-03, `results_fix/`, `scripts/fix_sweep_report.py`).
...
- `configs/config550_sl_ab.yaml` — Re_τ=550 A/B restart from the torChannel checkpoint (same 768×768×180 grid) for the wall-clock benchmark. NOTE: its torChannel restart path is currently stale.
- `bench/bench_interp.py` — M0 GPU cost of the SL machinery; break-even math in its docstring.

## Data layout (post-cleanup 2026-08-03)

- **No `results_*` dirs are kept in the repo root.** Archived stats from the M3 campaign live in `data/m3_stats/<run>.npz` (analysis scripts point there; `ref.npz` is the Eulerian reference). New sweeps write under a single parent, e.g. `results_fix/<run>/`. Field npz files are always disposable: the authoritative restart source is the CaNS checkpoint at `/home/giorgio/CaNS_DRL/run0_theory_big/data`, converted via `scripts/cans_to_npz.py` → `data/cans_run0_256x256x100.npz` (256×256×100, u_τ=0.0634, Re_τ=182).
- `output.t_snapshot` (sim time units) writes `fields_t*.npz` at fixed simulation-time thresholds — uniform in t⁺ across runs with different/adaptive dt (step-based `n_snapshot` is not).
- GPU is slurm-managed: check `squeue` before running; queue long runs with `sbatch` (see `run_fix_sweep.sh`); report quantities to the user in wall (+) units (this case: t⁺ = 11.78·t, ℓ⁺ = 183.9·ℓ).

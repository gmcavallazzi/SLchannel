...
The campaigns reported in \S\ref{sec:beyond} also exercised a first-order
...
$\dtp$ the net gain is $2$--$3.2\times$ per simulated time unit. Measured
end-to-end on the production closed-channel case at $256^{2}\times256$
(triquintic, $\dtp=0.25$): $336$\,ms/step against the Eulerian twin's
$203$\,ms/step at $\dtp\approx0.057$, i.e.\ $15.6$ versus $41.4$\,s per
simulated time unit, a net $\mathbf{2.65\times}$ --- where
...
--- is not in the literature. Those are the contributions, and the fidelity
map is now closed on both axes. The resolution branch: tricubic at
$\Delta x^{+}=11.8$ gives $\Ret=173.9$ ($-2.4\%$), at $\Delta x^{+}=8.8$
$\Ret=176.4$ ($-1.0\%$). The interpolation-order branch: triquintic at
$\Delta x^{+}=8.8$ recovers $\Ret$ to $+0.4\%$ of MKM with the peak
$u'_{\mathrm{rms}}^{+}$ within $-0.2\%$ --- inside sampling noise, and
better than the Eulerian twin on the same grid --- for $+2.6\%$ on the step
cost. Interpolation dissipation is therefore the entire statistics bias of the
method, and it is removable by order and resolution rather than intrinsic. What
remains intrinsic is temporal: the extrapolation-driven spectral floor above
$\dtp\approx0.22$ and the $17/9$ two-foot gain that ends the usable range at
$\dtp\approx0.30$.
